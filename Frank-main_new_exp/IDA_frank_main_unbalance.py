...
class Frank_unbalance:
    ...
    def train(self, X_frank_train, Y_frank_train):
        ...
        percentuali_dict,df_log = get_percentage_and_df(df_train,self.processed,self.target)
    ...
    def start(self):
        ...
        df_train = pd.DataFrame(self.X_frank_train)
        df_train[self.target] = self.Y_frank_train
        percentuali_dict = (df_train[self.target].value_counts(normalize=True) * 100).round(2).to_dict()
       ...
       #DataFrame iniziale
        ...
        for i in tqdm(range(len(self.X))):
            ...
            percentuali_dict = (df_log[self.target].value_counts(normalize=True) * 100).round(2).to_dict()
            model_balance.desired_dist = percentuali_dict
                ...
                if ideal_value is not None and user_truth != ideal_value and self.RULE: #User is consistent w.r.t. Ideal Rule
                    ...
                    if user_truth != prediction and self.SKEPT:
                        ...
                        if skepticism > 0.5:
                            ...
                            #cf_alibi,time_alibi,sparsity_alibi = get_examples_4(x,df_log,model_balance,self.target,self.cats)
                           ...
                           #print('Distanza tra x e esempi generati rispettante la regola : ',distances_x_generate_rule)
                            ...
                            #     #print('Controfattuale',counterfactual_lore) 
                            #     #print(distances_x_counter_rules)
                            ...
                            # cf_gs,time_gs,sparsity_gs =  get_examples_2(x,df_log,model_balance,self.target)#Controfattuali con Growing Sphere
                            ...
                            try:
                                cf_dice,time_dice,sparsity_dice =  get_counterfactuals_DICE(df_log,x,model_balance,self.cats,self.target)#Controfattuali con Growing Sphere
                            except:
                                ...
                                df_log = df_log.drop_duplicates(subset=cols_to_check, keep='last')
                            ...
                            if confirm in [0, "0", False]:
                                ...
                                decision = user_truth
                                #self.stats[user_truth]['user']['got'] += 1
                                df_similars,dicts = generate_similar(model_balance,cf_dice,df_log,self.target)
                                Y_h = []
                                new_dicts = []
                                # iter = 0
                                ...
                                for e in range(0,1):
                                    ...
                                    if Y_h!=[]:   
                                     ...
                                     df_log = pd.concat([df_log[:5], df_similars], ignore_index=True)         
            ...
            if i in self.fairness_records and self.GROUP:
                ...
                percentuali_dict,_ = get_percentage_and_df(df_train,self.processed,self.target)
                print(percentuali_dict)
            ...
            if i ==  (len(self.X)-1):
                ...
                with open(os.path.join(dir,'User_'+str(self.user)+self.name+str(self.model)+'times_DICE.txt'), 'w') as f:
                        for i, value in enumerate(times_DICE):
                            f.write(f"{i} {value}\n")            
