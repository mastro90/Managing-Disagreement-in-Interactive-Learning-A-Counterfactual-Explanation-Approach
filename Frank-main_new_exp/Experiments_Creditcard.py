...
# Scelta proporzione 
n_true = min(len(true_df), 100)  #  100 positivi
...
# positivi esclusi dal sottoinsieme di training
positivi_iterativo = df[df['target'] == True]
positivi_used = sampled_true  # quelli used nel training iterativo ridotto

#  positivi non used
positivi_non_used = positivi_iterativo[~positivi_iterativo.index.isin(positivi_used.index)]


df_test_clean = df_test[~df_test.index.isin(positivi_non_used.index)]

# Aggiunta  positivi scartati al test set
df_test_esteso = pd.concat([df_test_clean, positivi_non_used]).sample(frac=1, random_state=42)
...
# Filtro dei dizionari originali (con reconversione)
X_test_1 = []
Y_test_1 = []
...
#name = "user"+name_1
f = Frank_unbalance(name_1,bagging,user_model_3,balanced_iterative, target, protected, cats, rule_att, rule_value, RULE, PAST, SKEPT, GROUP, X_test_1, Y_test_1, EVA, N_BINS, N_VAR, MAX,X_frank_train_1,Y_frank_train_1,3)
