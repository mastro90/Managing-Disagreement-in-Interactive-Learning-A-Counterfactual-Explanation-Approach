...
# Rimozione  duplicati dal test set se ci fossero
df_test_clean = df_test[~df_test.index.isin(positives_non_used.index)]

# Aggiunta  positives scartati al test set
df_test_esteso = pd.concat([df_test_clean, positives_non_used]).sample(frac=1, random_state=42)

X_frank_train_1 = df_train.drop(columns=['target']).to_dict(orient='records')
Y_frank_train_1 = df_train['target'].tolist()
...
X_frank_train_1 = convert_dict_list_to_float32(X_frank_train_1)
...
adwin_b = ensemble.ADWINBoostingClassifier(model  = (sgt),n_models=3,seed=42 )
models = [ada,bagging,adwin_b]
X = balanced_iterative.loc[:, balanced_iterative.columns != target]
...
for x,y in zip(X_frank_train_1,Y_frank_train_1):
    ada.learn_one(x,y)
for x,y in zip(X,Y):
    y_pred = ada.predict_one(x)
    ada.learn_one(x,y_pred)
acc,f1,frank_cm =calculate_metrics(X_test,Y_test,ada)      
print(ada,'--->','acc:',acc,'f1:','--->',f1)  
...
for user in range(0,len(users)):
    ...
    f = Frank_unbalance(name,ada,users[user], balanced_iterative, target, protected, cats, rule_att, rule_value, RULE, PAST, SKEPT, GROUP, X_test, Y_test, EVA, N_BINS, N_VAR, MAX,X_frank_train_1,Y_frank_train_1,user)
