       ...
       # DataFrame iniziale
        ...
        for i in tqdm(range(len(self.X))):
            ...
            if record in list(self.processed.keys()): #Duplicated record
                ...
                if ideal_value is not None and user_truth != ideal_value and self.RULE: #User is consistent w.r.t. Ideal Rule
                    ...
                    if user_truth != prediction and self.SKEPT:
                        ...
                        if skepticism > 0.5  and self.simulated_user.believe():
                            cf_alibi,time_alibi,sparsity_alibi = get_counterexamples_proto(x,df_log,self.model,self.target,self.cats)#Controfattuali guidati da prototipi 
                            ...
                            df_validity=add_iteration(df_validity, i, x, cf_alibi, "Prototypes",self.model)
                            df_similars,dicts = generate_similar(self.model,x,df_log,self.target)#Genera esempi simili a un' istanza di partenza x 
                            ...
                            idx_dict_1 = filter_df_by_counter_rules(df_similars, counter_rules,self.model,x)#Filtra lo stori co in base alle controregole
                           ...
                           #print('Distanza tra x e esempi generati rispettante la regola : ',distances_x_generate_rule)
                            ...
                            cf_gs,time_gs,sparsity_gs =  get_counterfactuals_GS(x,df_log,self.model,self.target)#Controfattuali generati da  Growing Sphere
                            ...
                            cf_dice,time_dice,sparsity_dice =  get_counterfactuals_DICE(df_log,x,self.model,self.cats,self.target)#Controfattuali generati da DICE
                ...
                try:
                    self.acc.update(decision,prediction)
                    self.F1.update(decision,prediction)
            ...
            frank_acc,frank_cm,frank_f1 = calculate_metrics(self.X_test, self.Y_test, self.model)
            accuracy_score.append(frank_acc)
            f1_score.append(frank_f1)
            ...
            if i ==  (len(self.X)-1):
                ...
                "plausability":plausabilities_gs,
                 "method": "GS",  # Aggiungi il name del metodo
                "dataset": self.name
                }
                metrics_lore = {
                ...
                "proximity": proximities_LORE,
                 ...
                 "method": "LORE",  # Aggiungi il name del metodo
                "dataset": self.name
                }
                metrics_RL = {
                #"time_steps": time_lore,
                ...
                "proximity": proximities_RL,
                 ...
                 "method": "RL",  # Aggiungi il name del metodo
                "dataset": self.name
                }
                ...
                "proximity": proximities_DICE,
                 ...
                 "method": "DICE",  # Aggiungi il name del metodo
                "dataset": self.name
                }
