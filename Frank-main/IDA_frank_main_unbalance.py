...
class Frank_unbalance:
    ...
    def train(self, X_frank_train, Y_frank_train):
        ...
        for x, y in zip(self.X_frank_train, self.Y_frank_train):
            ...
            #xi_scaled = self.scaler.transform_one(x)
            ...
            #print(self.model._repr_html_)#Stampa del modello a ogni learn_one
            #print(self.model.draw()  )
        ...
        for x, y in zip(self.X_frank_train, self.Y_frank_train):
            ...
            #xi_scaled = self.scaler.transform_one(x)
            y_pred = model_balance.predict_one(x)
    ...
    def start(self):
        ...
        #copied_model = pickle.loads(pickle.dumps(self.model))
       ...
       #DataFrame iniziale
        ...
        for i in tqdm(range(len(self.X))):
            ...
            x = self.X[i]
            y = self.Y[i]
            print(df_log[self.target].value_counts())
            preds = [model_balance.predict_one(row) for row in df_log.drop(columns=[self.target]).to_dict(orient="records")]
            print('Percentuale : ',Counter(preds))
            percentage_dict = (df_log[self.target].value_counts(normalize=True) * 100).round(2).to_dict()
            #print(percentage_dict)
            #percentage_dict,df_log = get_percentage_and_df(df_train,self.processed,self.target)
            model_balance.desired_dist = percentage_dict
            #print(percentage_dict)
            ...
            prediction = model_balance.predict_one(x)
            ...
            if record in list(self.processed.keys()): #Duplicated record
                ...
                try:
                    pred_proba = model_balance.predict_proba_one(x)[prediction]
                except:
                    pred_proba = 0

                try:
                    user_proba = model_balance.predict_proba_one(x)[user_truth]
                except:
                ...
                if ideal_value is not None and user_truth != ideal_value and self.RULE: #User is consistent w.r.t. Ideal Rule
                    ...
                    if user_truth != prediction and self.SKEPT:
                        ...
                        preds = [model_balance.predict_one(row) for row in df_log.drop(columns=[self.target]).to_dict(orient="records")]
                        print('Percentuale : ',Counter(preds))
                        if skepticism > 0.5 and self.simulated_user.believe():
                            
                            cf_alibi,time_alibi,sparsity_alibi = get_counterexamples_proto(x,df_log,model_balance,self.target,self.cats)
                            print('Controfattuali alibi:',cf_alibi)
                            ...
                            df_validity=add_iteration(df_validity, i, x, cf_alibi, "RL",model_balance)
                            df_similars,dicts = generate_similar(model_balance,x,df_log,self.target)
                            #print(dicts)
                            #rules = generate_rules(model_balance,x,df_log,self.target)
                            #print('Rules:',rules)
                            df_log_1 = df_log.drop(self.target,axis=1)
                            #idx_dataset = filter_df_by_rule(df_log_1, rules,model_balance,x)
                            ...
                            counter_rules = generate_counter_rules(model_balance,x,df_log,self.target)
                            #print('Contro regole:',counter_rules)
                            #idx_dict_1 = filter_df_by_counter_rules(df_similars, counter_rules,model_balance,x)
                            time_LORE= time.time()-start_time
                            times_LORE.append(time_LORE)
                            #idx_dataset_1 = filter_df_by_counter_rules(df_log_1, counter_rules,model_balance,x)
                            ...
                            #idx_dict = filter_dict_by_rule(dicts, rules,x,model_balance)
                            #print('Esempi che rispettano la regola negli esempi generati : ',idx_dict)
                            #idx_dataset = filter_df_by_rule(df_similars, rules,model_balance,x)
                           ...
                           #print('Distanza tra x e esempi generati rispettante la regola : ',distances_x_generate_rule)
                            ...
                            idx_dict_1 = filter_dict_by_counter_rules(dicts, counter_rules,model_balance,x)
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    proximity_lore = distances_x_counter_rules[0][1]
                                except:
                                    X = df_log.drop(columns=[self.target]).to_dict(orient="records")
                                    min_d = float('inf')
                                    counterfactual_lore = None
                                    for x_h in X:
                                        if model_balance.predict_one(x) != model_balance.predict_one(x_h):      
                                            d = cdist(pd.DataFrame([x]), pd.DataFrame([x_h]), metric='euclidean').flatten()
                                            if d[0] < min_d:
                                                min_d = d[0]
                                                counterfactual_lore = x_h
                                                proximity_lore = min_d         
                            ...
                            df_validity=add_iteration(df_validity, i, x, counterfactual_lore, "LORE",model_balance)
                            cf_gs,time_gs,sparsity_gs =  get_counterfactuals_GS(x,df_log,model_balance,self.target)#Controfattuali con Growing Sphere
                            ...
                            df_validity=add_iteration(df_validity, i, x, cf_gs, "GS",model_balance)
                            cf_dice,time_dice,sparsity_dice =  get_counterfactuals_DICE(df_log,x,model_balance,self.cats,self.target)#Controfattuali con DICE
                            ...
                            df_validity=add_iteration(df_validity, i, x, cf_dice, "DICE",model_balance)
                            print('Inserimento nuova riga')

                            self.skept_count += 1
                            skepticisms.append(skepticism)
                            ...
                            if confirm == None: #The user is provided explanation of the model's decision. Our tests only focused on sythethic records
                                ...
                                ex_pro = dicts
                                for e in ex_pro:
                                    if model_balance.predict_one(e)!=model_balance.predict_one(x):
                                        ex_pro.remove(e)
                                ex_against = []
                ...
                self.accs.append(str(copy.deepcopy(acc_1)))
                self.F1s.append(str(copy.deepcopy(f1_)))
                model_balance.learn_one(x, decision)
            ...
            if relabel == True:
                ...
                X = df_log.loc[:, df_log.columns != self.target]
                X = list(X.to_dict(orient='index').values())
                Y = df_log[self.target].values.tolist()
            ...
            frank_acc,frank_f1,frank_cm = calculate_metrics(self.X_test, self.Y_test, model_balance)
            accuracy_score.append(frank_acc)
            f1_score.append(frank_f1)
            frank_cms.append(frank_cm)
            df_log = pd.concat([df_log,create_log(self.processed,self.target)],ignore_index=True)
            cols_to_check = [col for col in df_log.columns if col != self.target]
            df_log = df_log.drop_duplicates(subset=cols_to_check, keep='last')#Tolgo i duplicati
            ...
            if i ==  (len(self.X)-1):
                ...
                "proximity": proximities_LORE,
                 ...
                 "method": "LORE",  # Aggiungi il nome del metodo
                "dataset": self.name
                }
                metrics_RL = {
                #"time_steps": time_lore,
                ...
                "proximity": proximities_RL,
                 ...
                 "method": "RL",  # Aggiungi il nome del metodo
                "dataset": self.name
                }
                ...
                "proximity": proximities_DICE,
                 ...
                 "method": "DICE",  # Aggiungi il nome del metodo
                "dataset": self.name
                }
